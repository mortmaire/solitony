...
else tab[i][j][1]=tab[i][j][0]/trigger;
...
char name[30];
char name2[30];
sprintf(name,"../temp/%d%c.dat",getpid(),ffrk);
sprintf(name2,"../temp/%d%cS.dat",getpid(),ffrk);
...
file<<endl;
for(int i=0;i<100;i++)file2<<i<<"\t"<<stat[t1][i]<<endl;
file2<<endl<<endl;
}
file.close();
for(int i=0;i<100;i++){
file2<<i<<"\t"<<srednia[i]<<"\t"<<f(i)<<"\t"<<roznica[i]<<endl;
}
file2.close();
...
sprintf(script,READ(
set terminal png size 10240,768;
unset key;
set pm3d map;
set output "../temp/%d%c.png";
set title "T = %d, L = %d";
set xlabel "time";
set xrange [0:10000];
set ylabel "space position";
splot "%s" u 1:2:3;
set output "../temp/%d%cg.png";
set pal gray;
splot "%s" u 1:2:4;

),gpid,ffrk,(int)p.T,(int)p.L/100*100,n1,gpid,ffrk,n1);
...
char nn[20];
sprintf(nn,"%c.dat",ffrk);
fstream file(nn,ios::out|ios::ate);

int i=0;
int s=system("setterm -cursor off");
for(p0.T=10;p0.T<5000;p0.T+=5){
if(i++%8!=(int)(ffrk-'a'))continue;
ffal psi(p0);
psi.evolve();
double ss=psi.statystyka();
file<<p0.T<<"\t"<<ss<<endl;
gpid++;
}
file.close();
