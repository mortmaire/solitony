...
double g;
double dt;
parameters(double dx=0.1,double L=50,double gamma=0.01,double T=500,double mu=22.41,double g=0.1,double dt=0.0001):dx(dx),L(L),gamma(gamma),T(T),mu(mu),g(g),dt(dt){}
} p0;
...
double t0=t+time;
fstream file;
file.open("tmp.dat",ios::out);//czyszczę plik tmp.dat
file.close();
int i=0;
int I=1/p.dt/2000;
cout<<I<<endl;
...
file.open("tmp.dat",ios::out|ios::ate|ios::app);
file<<*this;
file.close();}
}
system("gnuplot colour.gp");
stringstream ss;
ss<<"mv out.png images/T="<<setw(3)<<setfill('0')<<p.T<<","<<setw(1)<<"p_dt="<<-log10(p.dt)<<",g="<<p.g<<",mu="<<p.mu<<".png";
system(ss.str().c_str());
cout<<ss.str()<<endl;
}
};
