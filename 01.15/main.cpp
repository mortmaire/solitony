...
bufor<<setw(4)<<setfill(' ')<<time<<"\t"<<x<<"\t"<<ctab[n]<<"\t"<<arg(psi->tab[n])<<"\t"<<soliton;
...
p0.T=5*(ffrk-'a')+i0*40;
if(ffrk=='a')cout<<p0.T<<endl;
for(int i=0;i<2;i++){
ffal psi;
psi.evolve();
char *file;
if(argc>1)file=argv[1];
else{ file=new char[20];
sprintf(file,"statystyka.txt");}
char str[100];
sprintf(str,"echo '%.5f\t%.5f' >> %s",p0.T,sol_sr,file);
system(str);
gpid++;
}i0++;
}
