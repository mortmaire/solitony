...
przebieg=new double*[M];
for(int i=0;i<M;i++)przebieg[i]=new double[N];
...
set title "T = %f";
set pal color;
set terminal png enhanced size 2*1024,768;
set output s."1.png";
set pm3d map;
splot s1 u 1:2:3;
set output s."2.png";
...
int N=2*p.L/p.dx+1;
int M=p.time/2;
for(int i=0;i<M;i++)delete[] przebieg[i];
delete[] przebieg;
...
}//ta pętla służy ustalenmiu max wart gęst.
