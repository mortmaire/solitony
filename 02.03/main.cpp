...
int length[]={50,100,200,500};
int temperature[]={1,1,20,10,100,50,500,500,2000,1000,6000};
int ln=sizeof(length)/sizeof(int);
int lt=sizeof(temperature)/sizeof(int);
p0.time=2000;
int i=0;
for(int i0=0;i0<ln;i0++){
p0.L=length[i0];

for(int i1=0;i1<lt/2*2;i1+=2)
for(p0.T=temperature[i1];
p0.T<temperature[i1+2];
p0.T+=temperature[i1+1]){
if(((i++)%8)!=ffrk-'a')continue;
ffal psi;
psi.evolve();
file<<p0.L<<"\t"<<p0.T<<"\t"<<sol_sr<<endl;
stringstream ss;ss<<p0.L<<"\t"<<p0.T<<endl;
cout<<ss.str();
gpid++;
