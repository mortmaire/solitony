...
if(argc<1){
cerr<<"Program generujący kolorowy przebieg ewolucji układu równań hybrydowych \n";
return 0;
}
fstream tem;
...
while(getline(tem,ciastko))out1<<p.t1*(i)<<ta<<ciastko<<endl;
out1<<endl;
tem.close();
...
ciastko;
tem.open("temp2.txt",fstream::in);
while(getline(tem,ciastko))out2<<p.t1*(i)<<ta<<ciastko<<endl;
out2<<endl;
tem.close();
