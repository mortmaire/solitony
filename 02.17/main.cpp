...
double *t2=tab[i][j];
complex z(t2[0],t2[1]);
if(((int)ffrk-'a')%2==1)t2[0]=abs(z);
else t2[0]=abs(z)*abs(z);//zwiększam x2
//t2[1]=arg(z);
//właściwie to mnie teraz nie obchodzisz
}
double tab2[N];
double max=0;
...
for(int i=0;i<100;i++)srednia[i]/=T/2;
...
for(int i=0;i<N;i++){file<<t1<<"\t"<<i*mnoznik<<"\t"<<tab[t1][i][0]<<"\t"<<tab[t1][i][1]<<endl;
...
file.close();
for(int i=0;i<100;i++){
file2<<i/100.*max<<"\t"<<srednia[i]<<"\t"<<f(i)<<"\t"<<roznica[i]<<endl;
}
file2.close();
...
unset pm3d;
nn2 = name."S.dat";
set output "../temp/".name."S.png";
set key;
plot nn2 u 1:2 i 10001 title "real distribution", nn2 u 1:3 i 10001 title "Gauss distribution", nn2 u 1:4 i 10001 title "Real - Gauss";
),gpid,ffrk,(int)p.T,(int)p.L/100*100,n1);

char name[30];sprintf(name,"../temp/%d%c.gp",gpid,ffrk);
fstream file(name,ios::out);
file<<script;
file.close();
char comm[30];
...
if(ffrk=='a'||ffrk=='b')p0.T=1000;
if(ffrk=='c'||ffrk=='d')p0.T=500;
if(ffrk=='e'||ffrk=='f')p0.T=2000;
if(ffrk=='g'||ffrk=='h')p0.T=10;
